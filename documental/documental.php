...
if (!isset($_SESSION['tipo']) == 'gerente' || ($_SESSION['tipo']) == 'sistemas' || ($_SESSION['tipo']) == 'gestor_riesgo'){
    ...
    <tr>
        ...
        <td class="align-middle"><?php $act = $row['actualizado'] == '0101-01-01' ? 'N/A' : $row['actualizado']; echo $act; ?></td>
        <td class="align-middle"><?php $rev = $row['revisado'] == '0101-01-01' ? 'N/A' : $row['revisado']; echo $rev; ?></td>
...
<?php
    ...
    <tr>
        ...
        <td class="align-middle"><?php $rev = $row['revisado'] == '0101-01-01' ? 'N/A' : $row['revisado']; echo $rev; ?></td>
        <td class="align-middle">
            <a onclick="loadId('../documental/documental_obsoletos/oldversions.php?id=','<?php echo $row['id'] ?>')" title="Ver versiones obsoletas" class="btn btn-secondary btn-sm"><i class="bi bi-collection-fill"></i></a>
        </td>
        <td class="align-middle">
            <a href="downloads.php?file=<?php echo $row['archivo'] ?>" title="Descargar documento" class="btn btn-primary btn-sm"><i class="bi bi-download"></i></a>
            <a onclick="loadId('../documental/visualizar.php?id=','<?php echo $row['id'] ?>')" data-toggle="tooltip" title="ir a visualizar" class="btn btn-success btn-sm"> <i class="bi bi-search"></i> </a>
            <a onclick="loadId('../documental/editar.php?id=','<?php echo $row['id'] ?>')" data-toggle="tooltip" title="Editar o actualizar datos" class="btn btn-success btn-sm"> <i class="bi bi-pencil-fill"></i> </a>
        </td>
    </tr>
    <?php
