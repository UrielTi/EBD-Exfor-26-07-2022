...
if (isset($_POST['update'])) {
			...
			if ($_FILES["img"]["size"] <= $limite_kb ) {
				if ($extension == 'jpeg' || $extension == 'jpg' || $extension == 'png' || $extension == 'webp') {
					...
					if ($extension == 'jpeg' || $extension == 'jpg') {
						...
						//Se destruye la imagen temporal
						imagedestroy($im);
						//Se toma el nombre de la imagen sin la ruta
					...
					echo "<script>alert(' La imágen no es del tipo JPEG, JPG, PNG, WEBP. '); window.location = 'editar.php?id=$id'</script>";
				}
			} else {
				echo "<script>alert(' La imágen pesa más de 20MB '); window.location = 'editar.php?id=$id'</script>";
			}
			$update_img = mysqli_query($conn, "UPDATE epp SET imagen='$imagen' WHERE id='$id' AND codigo='$codigo'") or die(mysqli_error($conn));

			if ($update_img) {
				if ($query_img['imagen'] != NULL) {
					unlink("./webp/" . $query_img['imagen']);
				}
	...
	if ($nombre != $row['nombre'] || $cantidad != $row['stock'] || $proveedor != $row['proveedor'] || $precio != $row['precio'] || $rotacion != $row['rotacion'] || $tipo_talla != $row['tipo_talla']) {
		$update = mysqli_query($conn, "UPDATE epp SET nombre='$nombre', stock='$cantidad', proveedor='$proveedor', precio='$precio', rotacion='$rotacion', tipo_talla='$tipo_talla' WHERE id='$id' AND codigo='$codigo'") or die(mysqli_error($conn));
	}
	...
	if ($updateTallaU || $insertEppTallas || $updateTalla28 || $updateTalla29 || $updateTalla30 || $updateTalla31 || $updateTalla32 || $updateTalla33 || $updateTalla34 || $updateTalla35 || $updateTalla36 || $updateTalla37 || $updateTalla38 || $updateTalla39 || $updateTalla40 || $updateTalla41 || $updateTalla42 || $updateTalla43 || $update_img || $update) {
