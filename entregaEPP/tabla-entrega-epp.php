...
if (isset($_POST['load_pending'])) {
    $consultEntregas = mysqli_query($conn, "SELECT id_empleado, fecha, COUNT(*) AS tanda, MAX(fecha) AS maxFecha, estado FROM entrega_epp WHERE estado = 0 GROUP BY id_empleado HAVING MAX(tanda) <= 1") or die(mysqli_error($conn));
...
?>
            <tr>
                ...
                <td class="align-middle">
                    <a href="excelEntregas.php?id=<?php echo $rE['id_empleado'] ?>" data-toggle="tooltip" title="Descargar constancia" class="btn btn-primary"><i class="bi bi-file-earmark-arrow-down"></i></a>
    ...
    if (isset($_POST['load_diligence'])) {
        $consultEntregas = mysqli_query($conn, "SELECT id_empleado, fecha, COUNT(*) AS tanda, MAX(fecha) AS maxFecha, estado FROM entrega_epp WHERE estado = 1 GROUP BY id_empleado HAVING MAX(tanda) <= 1") or die(mysqli_error($conn));
        while ($rE = mysqli_fetch_assoc($consultEntregas)) {
            $id_empleado = $rE['id_empleado'];
            $consultEmpleado = mysqli_query($conn, "SELECT nombres, primer_apellido, segundo_apellido, cedula FROM clientes WHERE id = '$id_empleado'") or die(mysqli_error($conn));
            while ($row = mysqli_fetch_assoc($consultEmpleado)) {
        ...
        ?>
                <tr>
                    ...
                    <td class="align-middle"><?php $echo = $rE['estado'] == 0 ? '<span class="badge rounded-pill text-bg-warning"><i class="bi bi-shield-exclamation"></i> Pendiente</span>' : '<span class="badge rounded-pill text-bg-success"><i class="bi bi-shield-check"></i> Diligenciado</span>';
                                                echo $echo ?></td>
                    <td class="align-middle">
                        <a href="<?php $echo2 = $rE['estado'] == 0 ? "excelEntregas.php?id=" . $id_empleado : "downloads.php?file=" . $documento; echo $echo2; ?>" data-toggle="tooltip" title="<?php $echo3 = $rE['estado'] == 0 ? 'Descargar constancia' : 'Descargar documento diligenciado'; echo $echo3; ?>" class="btn btn-primary"><i class="<?php $echo4 = $rE['estado'] == 0 ? 'bi bi-file-earmark-arrow-down' : 'bi bi-download'; echo $echo4; ?>"></i></a>
