...
$sql = mysqli_query($conn, "SELECT id, nombres, cedula, dias_ausentismo, fecha_inicio, fecha_final, estado FROM ausentismo");
while ($row = mysqli_fetch_assoc($sql)) { ?>
        <tr>
            <td><?php echo $row['id'] ?></td>
            <td><a onclick="loadData('bi bi-person-square','Visualización de la incapacidad','visualizar','<?php echo $row['id'] ?>')" href="" data-bs-toggle="modal" data-bs-target="#mimodal"><?php echo $row['nombres'] ?></a></td>
            <td><?php echo $row['cedula'] ?></td>
            <td><?php echo $row['dias_ausentismo'] ?></td>
            <td><?php echo $row['fecha_inicio'] ?></td>
            <td><?php echo $row['fecha_final'] ?></td>
            <td><?php echo $estados_aus_index[$row['estado']] ?></td>
            <td>
            <a onclick="loadData('bi bi-person-square','Visualización de la incapacidad','visualizar','<?php echo $row['id'] ?>' )" href="" data-toggle="tooltip" title"Ver" class="btn btn-sm btn-sm-info" data-bs-toggle="modal" data-bs-target="#mimodal"> <i class="bi bi-search"></i> </a>
